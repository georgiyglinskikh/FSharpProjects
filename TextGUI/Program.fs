...
let get path regex = path |> http |> RegEx regex
...
let main argv =
    ...
    let form = new Form(Size = Size(800, 600), Text="Частота слов / Word frequency", FormBorderStyle = FormBorderStyle.FixedDialog)
    ...
    let textBox = new TextBox(Location = Point(10, 43), Size = Size(760, 500), ReadOnly = true, Multiline = true, ScrollBars = ScrollBars.Vertical)
    ...
    getTextButton.Click.Add(fun args -> 
        if authorBox.Items.Contains(authorBox.Text) && bookBox.Items.Contains(bookBox.Text) then
            ...
            let wordsByDecending = (getUrlBooksText (authorBox.Text.ToUpper()) bookBox.Text) |> getText |> Array.map (fun el -> el.ToLower()) |> Array.groupBy id |> Array.sortByDescending (fun el -> (snd el).Length) |> Array.map (fun a -> ((fst a), (snd a).Length))
            textBox.Text <- Array.fold (fun a b -> a + (b |> fst |> string) + " - " + (b |> snd |> string) + "\r\n") " " wordsByDecending
        else 
            textBox.Enabled <- false
            textBox.Text <- ""
    )
    ...
    bookBox.SelectedValueChanged.Add(fun args -> 
